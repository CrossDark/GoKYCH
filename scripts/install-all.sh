...
# Without this, `ReadWritePaths` under `ProtectSystem=strict` would block
# the rename with EROFS even though deploy owns the file. /opt/gokych/data
# is the runtime database/uploads/avatars directory; widen it (default).
...
# sudoers: let the deploy user ask systemd to restart the service without
# a password. This is what enables the in-app "auto-restart after update"
# flow — the Go process detects it's running under systemd and spawns
# `sudo systemctl restart gokych.service` instead of the old in-process
# `syscall.Exec` (which silently failed in the deploy user's restricted
# service environment, forcing operators to run the same command by hand
# after every update).
#
# The command path is resolved at install time so the rule matches the
# actual systemctl binary on the host (different distros ship it at
# /bin/systemctl vs /usr/bin/systemctl).
SYSTEMCTL_BIN="$(command -v systemctl)"
if [[ -z "$SYSTEMCTL_BIN" ]]; then
  warn "systemctl 不在 PATH 上 — 自更新后无法自动重启 (fallback 到 in-process exec)"
else
  log "写 sudoers 规则（deploy 无密码重启 gokych.service）..."
  SUDOERS_FILE="/etc/sudoers.d/gokych-restart"
  cat > "$SUDOERS_FILE" <<SUDOERS
# Auto-generated by install-all.sh. Lets the gokych backend ask systemd
# to restart itself after a successful self-update (POST /api/admin/update/apply).
# Scoped to the systemctl restart verbs only — no other root commands.
...
SUDOERS
  ...
  if visudo -c -f "$SUDOERS_FILE" >/dev/null 2>&1; then
    ok "sudoers 规则就位 ($SUDOERS_FILE)"
  else
    warn "sudoers 语法校验失败 — 自更新后请手动 sudo systemctl restart gokych"
    rm -f "$SUDOERS_FILE"
  fi
fi
